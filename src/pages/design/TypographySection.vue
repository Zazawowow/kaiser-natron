...
// Fluid section-headline clamps shared by the full-bleed sections.
const headlineScale = ['display', 'headline-lg', 'headline-md']
</script>
  ...
  >
    <section class="grid md:grid-cols-2 gap-6">
      <Card tone="paper">
        ...
        <p class="font-display text-5xl font-normal leading-[1.05] mb-3">Zeitung</p>
      ...
      <Card tone="paper">
        ...
        <p class="font-sans text-5xl font-medium leading-[1.05] mb-3">Zeitung</p>
    ...
    <section>
      <h2 class="eyebrow mb-6">{{ t('ds.heading.headlineScale') }}</h2>
      <div class="divide-y divide-line rounded-md border border-line bg-paper px-6">
        <div
          v-for="size in headlineScale"
          :key="size"
          class="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-8 py-5"
        >
          <code class="font-mono text-[11px] text-muted w-36 shrink-0">text-{{ size }}</code>
          <span :class="`font-display font-normal leading-[1.05] text-${size}`">{{ t('ds.typography.sample') }}</span>
        </div>
      </div>
    </section>
  </SectionShell>
